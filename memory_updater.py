...
try:
    ...
    try:
        # Try alternative path for Blob if the first one fails
        from langchain_community.document_loaders.parsers.html.bs4 import BS4HTMLParser
        from langchain_community.document_loaders.blob_loaders.schema import Blob
    except ImportError:
        # Fallback/mock if specific langchain version differs
        BS4HTMLParser = object
        Blob = object
...
class CustomHTMLParser(BS4HTMLParser):
    ...
    def lazy_parse(self, blob: Blob):
        ...
        log.warning(f"CustomHTMLParser LAZY_PARSE triggered for: {blob.source} ({blob.mimetype})")
        ...
        if self.ignore_display_none:
            ...
            for div in divs:
                ...
                if should_remove:
                    ...
                    log.warning(json.dumps({
                        "component": "ccat_memory_updater",
                        "event": "html_parser_element_removed",
                        "data": {
                            "source": blob.source.replace("/home/davide/Documents/core/", "") if hasattr(blob, 'source') else "unknown",
                            "element": "div",
                            "reason": "display:none"
                        }
                    }))
...
@hook(priority=100) # Run late to ensure we override others
def rabbithole_instantiates_parsers(file_handlers: dict, cat) -> dict:
    ...
    try:
        # Debug log
        log.info("rabbithole_instantiates_parsers hook called. Attempting to load settings.")
        ...
        if ignore_display_none:
            # Replace the HTML handler
            file_handlers["text/html"] = CustomHTMLParser(ignore_display_none=True)
            log.info(json.dumps({
                "component": "ccat_memory_updater",
                "event": "html_parser_replaced",
                "data": {
                    "parser": "CustomHTMLParser",
                    "reason": "ignore_display_none=True"
                }
            }))
        else:
            log.info("Not replacing HTML parser because ignore_display_none is False")

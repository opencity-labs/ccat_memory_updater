...
def scrapycat_after_ingestion(context_data: dict, cat: StrayCat):
    ...
    available = (
        settings.get("dietician_scrapycat_middleman", False)
        and dietician_plugin
        and scrapycat_plugin
    )
    ...
    try:
        ...
        try:
            ...
            remove_documents_by_metadata = getattr(
                dietician_module, "remove_documents_by_metadata", None
            )
        except ImportError:
            log.error(
                json.dumps(
                    ...
                    try:
                            ...
                            json.dumps(
                                {
                                    ...
                                    "data": {"attempt": attempt, "url": failed_url},
                                }
                            )
                        )
                        ...
                        if is_permanent:
                                ...
                                json.dumps(
                                    {
                                        ...
                                        "data": {"url": failed_url, "error": str(e)},
                                    }
                                )
                            )
                            ...
                            retry_results["errors"].append(
                                f"Permanent error for {failed_url}: {str(e)}"
                            )
                            ...
                            if attempt == max_attempts:
                                retry_results["errors"].append(
                                    f"Exhausted retries for {failed_url}: {str(e)}"
                                )
                                log.error(
                                    json.dumps(
                                        {
                ...
                json.dumps(
                    {
                        ...
                        "data": {"skipped_count": len(failed_pages)},
                    }
                )
            )
        ...
        exclude_sources = (
            updated_scraped + context_data.get("ignored_pages", []) + remaining_failed
        )
                ...
                json.dumps(
                    {
                        ...
                        "data": {"url": removed_url, "session_id": session_id},
                    }
                )
            )
        ...
        if removed_urls:
            try:
                ...
                anonymizer_module = importlib.import_module(
                    "cat.plugins.ccat_anonymizer.allowedlist"
                )
                        ...
                        json.dumps(
                            {
                                ...
                                "data": {"removed_sources_count": len(removed_urls)},
                            }
                        )
                    )
            except ImportError:
        ...
        if (
            ...
            or cleanup_result["vector_removed_count"] > 0
        ):
            cat.send_ws_message(
